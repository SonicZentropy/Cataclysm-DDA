...
namespace {
    ...
    bool _trim_overmapbuffer(std::filesystem::path const& dep, tripoint_range<tripoint> const& oms) {
        std::string const fname = dep.extension() == zzip_suffix
                                      ? // NOLINT(cata-u8-path)
                                      dep.filename().replace_extension("").generic_u8string()
                                      : // NOLINT(cata-u8-path)
...
namespace debug_menu {
    ...
    static std::vector<trait_id> setup_traits{
        trait_DEBUG_BIONICS, trait_DEBUG_CLAIRVOYANCE, trait_DEBUG_CLOAK,
        trait_DEBUG_HS, trait_DEBUG_LIGHT, trait_DEBUG_LS, trait_DEBUG_MANA, trait_DEBUG_MIND_CONTROL, trait_DEBUG_PHASE_MOVEMENT,
    ...
    static void spell_description(
        ...
        // Class: Spell Class
        description << string_format(_("Class: %1$s"), colorize(spl.spell_class() == trait_NONE ? _("Classless") : chrc.mutation_name(spl.spell_class()),
                                                                yellow)) << "\n";
    ...
    static void debug_menu_force_temperature() {
        ...
        if (tempmenu.ret == 0) {
            ...
            auto ask = [](const std::string& unit, std::optional<float> current) {
                int ret = !!current ? static_cast<int>(std::round(*current)) : static_cast<int>(std::round(get_weather().temperature.value()));
                return query_int(ret, !!current, _("Set temperature to?  [%s]"), unit) ? std::optional<float>(static_cast<float>(ret)) : current;
            };
    ...
    static void change_weather() {
        ...
        weather_menu.text = _("Select new weather pattern:");
        weather_menu.addentry(0, true, MENU_AUTOASSIGN, weather.weather_override == WEATHER_NULL ? _("Keep normal weather patterns") : _("Disable weather forcing"));
    ...
    static void wind_direction() {
        ...
        weather_manager& weather = get_weather();
        wind_direction_menu.addentry(0, true, MENU_AUTOASSIGN, weather.wind_direction_override ? _("Disable direction forcing") : _("Keep normal wind direction"));
        int count = 1;
    ...
    static void wind_speed() {
        ...
        weather_manager& weather = get_weather();
        wind_speed_menu.addentry(0, true, MENU_AUTOASSIGN, weather.wind_direction_override ? _("Disable speed forcing") : _("Keep normal wind speed"));
        int count = 1;
