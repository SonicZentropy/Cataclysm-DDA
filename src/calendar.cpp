...
bool calendar::once_every(const time_duration& event_frequency) {
    if (event_frequency == 0_turns) {
        // zero frequency is invalid; emit a debug warning and return true (every turn)
        // or return false (never) — either is better than crashing
        debugmsg("once_every called with zero duration");
        return true;
    }
