...
std::string options_manager::cOpt::getDefaultText(const bool bTranslated) const {
    if (sType == "string_select") {
        ...
        const std::string defaultName = iter == vItems.end() ? std::string() : bTranslated ? iter->second.translated() : iter->first;
    ...
    add_option_group("general", Group("auto_feat_opts", to_translation("Auto features options"),
                     ...
                     [&](const std::string& page_id) {
                         ...
                         add("AUTO_FORAGING", page_id, to_translation("Auto foraging"),
                             to_translation(
                                 "Action to perform when 'Auto foraging' is enabled.  Bushes: Only forage bushes.  - Trees: Only forage trees.  - Crops: Only forage crops.  - Everything: Forage bushes, trees, crops, and everything else including flowers, cattails etc."),
    ...
    add_option_group("general", Group("safe_mode_opts", to_translation("Safe mode options"),
                     ...
                     [&](const std::string& page_id) {
                         add("SAFEMODEPROXIMITY", page_id, to_translation("Safe mode proximity distance"),
                             to_translation(
                                 "If safe mode is enabled, distance to hostiles at which safe mode should show a warning.  0 = Max player view distance.  This option only has effect when no safe mode rule is specified.  Otherwise, edit the default rule in Safe mode manager instead of this value."),
    ...
    add_option_group("general", Group("misc_general_opts", to_translation("Misc Options"),
                     ...
                     [&](const std::string& page_id) {
                         ...
                         add("CIRCLEDIST", page_id, to_translation("Circular distances"),
                             to_translation(
                                 "If true, the game will calculate range in a realistic way: light sources will be circles, diagonal movement will cover more ground and take longer.  If false, everything is square: moving to the northwest corner of a building takes as long as moving to the north wall."),
                             true
                         );
...
void options_manager::add_options_interface() {
    ...
    add("DIAG_MOVE_WITH_MODIFIERS_MODE", "interface",
        ...
        */
        to_translation("Allows diagonal movement with cursor keys using CTRL and SHIFT modifiers.  Diagonal movement action keys are taken from keybindings, so you need these to be configured."), {
...
void options_manager::add_options_world_default() {
    ...
    add_option_group("world_default", Group("item_category_spawn_rate",
                                            ...
                                            to_translation(
                                                "Spawn rate for item categories.  Values less than or equal to 1.0 represent a chance for items from category in question to spawn at all.  Values more than 1.0 represent amount of additional items from category in question to spawn.  Set to 0.0 to forbid spawning items from category in question.")),
                     [&](const std::string& page_id) {
    ...
    add("SHOW_ALL_ITEMS", "world_default", to_translation("Show All Items"),
        to_translation("Controls whether items are shown even when in containers"), false);
    add("ALLOW_TORN_POCKETS", "world_default", to_translation("Allow Torn Pockets"),
        to_translation("Controls whether your pockets can tear and dump their contents without you noticing."), true);
    add("DISABLE_WEARINESS", "world_default", to_translation("Disable Weariness"),
        to_translation("Controls whether weariness is used.  When true, Weariness is permanently set to zero.  Sleepiness and stamina are still a thing."), false);
    add("DISABLE_FOCUS", "world_default", to_translation("Disable Focus"),
        to_translation("Controls whether focus is used.  When true, Focus is permanently locked at 100. Recommend to take slower learning trait to offset."), false);
    add("REMOVE_RIOT_FIRES", "world_default", to_translation("Remove Riot Fires"),
        to_translation("Riot damage can still occur, but random places being on fire won't happen"), false);
    add("ALWAYS_USE_DEBUG_MAP", "world_default", to_translation("Always Use Debug Map"),
        to_translation("Replaces the overmap with the debug one.  It's only here as a dev convenience, you probably don't want to turn this one on"), false);
    add("NO_DISABLING_ACHIEVEMENTS", "world_default", to_translation("No Disabling Achievements"),
        to_translation("Disables debug mode from disabling achievements"), false);
...
std::string options_manager::show(bool ingame, const bool world_options_only, bool with_tabs) {
    ...
    ui.on_redraw([&](const ui_adaptor&) {
        ...
        //Draw options
        for (int i = iStartPos;
             i < iStartPos + (iContentHeight > static_cast<int>(visible_items.size()) ? static_cast<int>(visible_items.size()) : iContentHeight); i++) {
...
void options_manager::deserialize(const JsonArray& ja) {
    for (JsonObject joOptions : ja) {
        ...
        std::vector<std::string> removed_options = {
            "DISTANCE_INITIAL_VISIBILITY", "FOV_3D_Z_RANGE", "SAFEMODE",
